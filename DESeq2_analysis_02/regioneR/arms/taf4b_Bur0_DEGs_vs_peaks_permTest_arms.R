...
# Use permutation test function in regioneR to determine if DEGs overlap
# features of interest (e.g., peaks) more than expected by chance

# Usage:
# taf4b_Bur0_DEGs_vs_peaks_permTest_arms.R taf4bVCol_Bur0VCol_.01_lfcShrink_Chr_Sig.01_downRegGeneIDs.txt 
...
plotDir <- "/projects/ajt200/BAM_masters/RNAseq_meiocyte_Feng/DESeq2_analysis_02/regioneR/arms/plots/DEGsVsFeatures/"
...
#                  c(chrStart, pericenStart-999, chrLens-999),
#                  c(chrStart+999, pericenEnd+999, chrLens)))
...
ptDEGsOtherPerChrom <- lapply(seq_along(grl), function(x) {
  permTest(A = DEGsGR, B = grl[[x]], genome = genome, mask = mask,
...
save(ptDEGsOtherPerChrom,
     file = paste0(outDir,
                   "permTest_",
                   DEGsFileBasename,
                   "_REC8_nucleRnucs_rangernucs_SPO11_H3K4me3_H3K9me2_COs_noMinWidth.RData"))
...
write.table(ptDEGsOtherPerChromDataFrame,
            file = paste0(outDir,
                          "permTest_", 
                          DEGsFileBasename,
                          "_REC8_nucleRnucs_rangernucs_SPO11_H3K4me3_H3K9me2_COs_noMinWidth_DataFrame.txt"),
            sep = "\t", row.names = F)
...
for(i in 1:length(ptDEGsOtherPerChrom)) {
  ...
  plot(ptDEGsOtherPerChrom[[i]], main = paste0(DEGsFileBasename, " vs ", otherNames[i], " on chromosome arms"), xlab = "Number of overlaps", ylab = "Relative frequency")
  dev.off()
  ...
  lz_1kb <- localZScore(pt = ptDEGsOtherPerChrom[[i]], A = DEGsGR, B = grl[[i]],
                        window = 1000, step = 50, count.once = TRUE)
  lz_10kb <- localZScore(pt = ptDEGsOtherPerChrom[[i]], A = DEGsGR, B = grl[[i]],
                         window = 10000, step = 500, count.once = TRUE)
  lz_custom <- localZScore(pt = ptDEGsOtherPerChrom[[i]], A = DEGsGR, B = grl[[i]],
                           window = 10*mean(width(DEGsGR)), step = mean(width(DEGsGR))/2, count.once = TRUE)

  pdf(file = paste0(plotDir, otherNames[i], "_localZscore_permTest_nperm10000_", DEGsFileBasename, "_arms_w1kb_s50bp_w10kb_s500bp_w25kb_s1254bp_perChrom.pdf"))
  par(mar=c(5.1, 4.1, 4.1, 2.1))
  plot(lz_1kb, main = paste0(DEGsFileBasename, " vs ", otherNames[i], " on chromosome arms (1-kb shift)"))
  mtext(side = 3, at = 2, text = paste0(DEGsFileBasename, " vs ", otherNames[i], " on chromosome arms (1-kb shift)"))
  plot(lz_10kb, main = paste0(DEGsFileBasename, " vs ", otherNames[i], " on chromosome arms (10-kb shift)"))
  mtext(side = 3, at = 2, text = paste0(DEGsFileBasename, " vs ", otherNames[i], " on chromosome arms (10-kb shift)"))
  plot(lz_custom, main = paste0(DEGsFileBasename, " vs ", otherNames[i], " on chromosome arms (~7-kb shift)"))
  mtext(side = 3, at = 2, text = paste0(DEGsFileBasename, " vs ", otherNames[i], " on chromosome arms (~7-kb shift)"))
  dev.off()
}
